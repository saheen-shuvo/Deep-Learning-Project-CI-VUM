...
def main():
    ...
    model.save(os.path.join(OUT_DIR, "model_sent_mlp.keras"))
    save_plots(hist, prefix="SENT_MLP")
    eval_and_save(model, X_test, np.array(y_test), prefix="SENT_MLP")

    with open(os.path.join(OUT_DIR, "sent_mlp_history.json"), "w", encoding="utf-8") as f:
        json.dump(hist.history, f, indent=2)

    print("DONE: Sentiment MLP finished. Check outputs/sentiment/")
